...
parser.add_argument("--num_cores", type=int, help="Number of cores", required=True)
parser.add_argument("--vlen", type=int, help="Length of each vector registers in bits", required=True)
...
class HighPerformanceArmBoard(ArmBoard):
    ...
    def _pre_instantiate(self):
        ...
        # enable prefetcher
        for ccd in self.cache_hierarchy.core_complexes:
            for cluster in ccd.core_clusters:
                cluster.l1_cache.enable_prefetch = True

        # enlarge LSQ
        num_cores = self.get_processor().get_num_cores()
        lsq_entries = 32 * 16 // num_cores
        for switch in self.processor._switchable_cores['switch']:
            switch[0].core.LQEntries = lsq_entries
            switch[0].core.SQEntries = lsq_entries
...
board.set_kernel_disk_workload(
    ...
    disk_image=DiskImageResource("/projects/gem5/hn/DISK_IMAGES/arm64sve-hpc-2204.img"),
